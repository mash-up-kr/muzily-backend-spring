...
class PlaylistAcceptanceTest : AcceptanceTest() {
    ...
    fun `재생목록을 조회한다`() {
        ...
        // when
        val response = `재생목록 조회 요청`(`SNS 계정 인증`.token, 방.playlistId!!)
    ...
    fun `계류중인 재생목록 항목들을 조회한다`() {
        ...
        `방 재생목록 항목 신청 요청값들`.forEach {`재생목록 항목 신청되어 있음`(`익명 세션`, 방.roomId, it)}

        // when
        val response = `계류중인 재생목록 항목 목록 조회 요청`(`SNS 계정 인증`.token, 방.playlistId!!)
    ...
    fun `익명 회원은 계류중인 재생목록 항목들을 조회할 수 없다`() {
        ...
        // when
        val response = `계류중인 재생목록 항목 목록 조회 요청`(`익명 회원가입 인증`.token, 방.playlistId!!)
